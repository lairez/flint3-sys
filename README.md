...
- `use-system-lib`: Links dynamically against a system FLINT discovered with `pkg-config`. This uses the checked-in bindings unless `force-bindgen` is also enabled. The detected FLINT major/minor version must match the bundled FLINT version when using checked-in bindings.
...
For these reasons, `use-system-lib` uses the checked-in bindings by default and checks that the system FLINT major/minor version matches the bundled FLINT version. Enable `force-bindgen` as well only if you intentionally want to regenerate bindings from the system headers.
...
2. Run `KEEP_BINDGEN_OUTPUT=1 cargo build -F force-bindgen` to regenerate `./bindgen/flint.rs`.
3. Test thoroughly.
